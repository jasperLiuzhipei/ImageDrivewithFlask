...
@logs_bp.route("/export", methods=["GET", "POST"])  # support GET for window.open
@jwt_required()
def export_logs():
    uid = int(get_jwt_identity())
    action = (request.args.get("action") or "").strip()
    ...
    text = "\n".join(rows) + "\n"
    return Response(text, mimetype="text/csv", headers={"Content-Disposition": "attachment; filename=logs.csv"})
