<template>
  <el-card>
    <div class="title"><el-icon><Document /></el-icon><span>OCR Search</span></div>
    <el-input v-model="q" placeholder="Enter OCR keywords (e.g., invoice, date)" style="max-width: 520px" @keyup.enter.native="onSearch">
      <template #append>
        <el-button type="primary" :loading="loading" @click="onSearch">Search</el-button>
      </template>
    </el-input>
    <el-empty v-if="!rows.length && inited" description="No results. Try other keywords or ingest OCR first" style="margin-top: 16px;" />
