<template>
  <el-header class="nav">
    ...
    <el-menu mode="horizontal" :ellipsis="false" class="menu" router>
      ...
      <el-menu-item index="/search/text">Text Search</el-menu-item>
      <el-menu-item index="/search/ocr">OCR Search</el-menu-item>
      <el-menu-item index="/similar">Similarity Search</el-menu-item>
    ...
    <div class="right">
      <template v-if="auth.user">
        <el-dropdown>
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="openChange">Change Password</el-dropdown-item>
              <el-dropdown-item @click="onLogout">Logout</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
        <el-dialog v-model="showChange" title="Change Password" width="420px">
          <el-form :model="form" label-width="96px">
            <el-form-item label="Current Password"><el-input v-model="form.old" show-password /></el-form-item>
            <el-form-item label="New Password"><el-input v-model="form.n1" show-password /></el-form-item>
            <el-form-item label="Confirm Password"><el-input v-model="form.n2" show-password /></el-form-item>
          </el-form>
          <template #footer>
            <div style="display:flex; justify-content:flex-end; gap:8px;">
              <el-button @click="onCancel">Cancel</el-button>
              <el-button type="primary" :loading="loading" @click="onConfirm">Confirm</el-button>
            </div>
          </template>
        </el-dialog>
      </template>
...
async function onConfirm() {
  if (!form.old || !form.n1 || !form.n2) return ElMessage.warning('Please complete all required fields')
  if (form.n1 !== form.n2) return ElMessage.warning('New passwords do not match')
  loading.value = true
  ...
  try {
    ...
    ElMessage.error(e?.response?.data?.message || 'Failed to update password')
  } finally {
    loading.value = false
  }
}
